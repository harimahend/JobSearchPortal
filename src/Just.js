...
function Just (){
  ...
  useEffect(() => {
    axios.get('https://643aacc290cd4ba563fee2b3.mockapi.io/fetchdata')
      .then(response => {
  ...
  return (
    ...
    <div className="grid grid-cols-2 gap-4 w-[830px] h-[320px]">
      {jobs.map((job) => (
        <div
          key={job.id}
          className={` p-4 bg-gray-100 border rounded-md grid-row-${((job.id-1)%2)+1}`}
        >
      ...
      <p> Experience&#40;{job.Experiencemin} -{job.Experiencemax}&#41;</p>
      <div className='h-2'>&nbsp;</div>
